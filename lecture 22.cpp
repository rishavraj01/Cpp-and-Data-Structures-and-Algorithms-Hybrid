...
int main()
{
	...
	int *p;
	p=&a;
	a=2;  //changing the value, but the address of a is still the same, so the pointer will still point to the same a no matter how the a is being changed
	*p=3;
	cout<<"The value of a is(from the actual variable type)"<<a<<endl;
	cout<<"The value of from the pointer using *(astreix/ indirection operator) "<<*p<<endl;
	return 0;
...
int main()
{
	...
	q=&b;
	a=2;
	b=5;  //changing the value, but the address of a is still the same, so the pointer will still point to the same a no matter how the a is being change
	c = *p+*q;
